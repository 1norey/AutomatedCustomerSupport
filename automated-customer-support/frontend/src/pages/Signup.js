...
export default function Signup() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      navigate(role === "admin" || role === "agent" ? "/dashboard" : "/try");
    } catch (err) {
      ...
      <div className="w-full max-w-md bg-white/10 p-8 rounded-xl shadow-xl border border-white/20">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <input type="email" name="email" placeholder="Email" onChange={handleChange}
            className="w-full p-3 rounded bg-white/5 text-white border border-white/20" />
        ...
        <p className="text-center mt-4 text-sm text-gray-300">
          Already have an account? <Link to="/" className="text-[#DBAAA7] hover:underline">Log In</Link>
        </p>
      </div>
    </div>
  );
